...
exports.run = async (client, message, args, config) => {
	...
	if (action === 'off') {
		...
		const channelId = channelsObj["📁・message-logs"];
		if (channelId) {
			const channel = message.guild.channels.cache.get(channelId);
			if (channel) await channel.delete().catch(() => { });
			delete channelsObj["📁・message-logs"];
			db.run(
				`INSERT OR REPLACE INTO logs (guild, channels) VALUES (?, ?)`,
				[message.guild.id, JSON.stringify(channelsObj)]
			);
			return message.reply("Les logs des messages sont désactivé.");
		} else {
			return message.reply("Pas de logs des messages configuré.");
		}
	}
	...
	channelsObj["📁・message-logs"] = finalChannel.id;
